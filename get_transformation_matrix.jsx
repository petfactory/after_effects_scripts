...
// create the data null
// to this node I will add a 3D Point Control effect for x, y, z axis and pos
var data_null =  comp.layers.addNull();
data_null.name = "data_null"
...
for (var i = 0; i < num_sel; i++)
{
    ...
    if (sel_layers[i] instanceof AVLayer  && sel_layers[i].threeDLayer)
    {
        ...
        var pos = data_null.property("Effects").addProperty("3D Point Control");
        pos.name = "p " + sel_layers[i].name
        var exp_pos = "L1 = thisComp.layer(\"" + sel_layers[i].name+"\");\np = L1.toWorld([0,0,0]);\n";
        pos.property("3D Point").expression = exp_pos;

        effect_array.push(pos)
    }
